const data = [
  ...
  const lowerstSallary = data.findIndex(
    (user) => user.sallary > 0 && user.sallary < 2000
  );

  console.log(lowerstSallary);

  data[lowerstSallary].sallary += 200;
  ...
  // 7 - reduce 
  const sallarlessSun = data.reduce((totalSallary, user) => totalSallary += user.sallary,0 );

  console.log(sallarlessSun);
